...
- `minSdk` 33, `compileSdk` 37, `targetSdk` 37 : communs à la suite (`CiaoSdk` dans `build-logic`)
...
- Nom affiché de l'app : **C!ao Galerie** (`app_name`) — C!ao est le nom de la suite d'applications (Galerie, et d'autres à venir), Galerie identifie cette app-ci au sein de la suite. `applicationId`/package `dev.ybdn.ciao.galerie` (convention commune à la suite, `CLAUDE.md` racine), fichier DataStore `galerie_settings`, base `galerie.db`, canal de notification `galerie_transfer`, classes `Galerie*`, thème `Theme.Galerie`.
...
- Build debug interprété très lent sur l'émulateur : `adb shell cmd package compile -m speed -f dev.ybdn.ciao.galerie` avant de juger les performances.
