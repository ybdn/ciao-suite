...
import dev.ybdn.ciao.galerie.domain.model.DateShift
...
import dev.ybdn.ciao.galerie.domain.repository.MediaDetailsReader
...
class EditMetadataUseCaseTest {
    ...
    fun `an identical file already in the target folder is reused`() = runTest {
        ...
        val plan = dev.ybdn.ciao.galerie.domain.util.ExifWritePlan.from(
            MetadataChanges(dateShift = DateShift(days = 1)),
            dev.ybdn.ciao.galerie.domain.model.CaptureTimestamp(LocalDateTime.of(2025, 4, 21, 10, 0)),
        )
