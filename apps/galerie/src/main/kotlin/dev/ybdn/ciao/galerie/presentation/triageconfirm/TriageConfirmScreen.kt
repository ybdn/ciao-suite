package dev.ybdn.ciao.galerie.presentation.triageconfirm
...
import dev.ybdn.ciao.galerie.presentation.gallery.GalleryEventsEffect
import dev.ybdn.ciao.galerie.presentation.gallery.GalleryTile
...
fun TriageConfirmScreen(
    ...
    val viewModel = galerieViewModel { container, app -> TriageConfirmViewModel(container, app) }
        ...
        if (plan.ssdOnly.isNotEmpty()) {
            fullWidth("ssd-header") {
                SectionHeader(
                    ...
                    NeoNotice(stringResource(R.string.triage_confirm_ssd_warning), tone = NeoTone.Danger)
                    if (!ssdAvailable) NeoNotice(stringResource(R.string.triage_confirm_plug_ssd), tone = NeoTone.Warning)
                }
            }
...
private fun SsdDeletionDialog(count: Int, onConfirm: () -> Unit, onDismiss: () -> Unit) {
    Dialog(onDismissRequest = onDismiss) {
        NeoCard(modifier = Modifier.padding(8.dp)) {
            ...
            NeoNotice(pluralStringResource(R.plurals.gallery_delete_ssd_confirm, count, count), tone = NeoTone.Danger)
            NeoButton(stringResource(R.string.gallery_delete_ssd_confirm_button), onClick = onConfirm, tone = NeoTone.Danger)
