...
- Prise en charge de `moveDocument` par le fournisseur de stockage du SSD USB (repli copie + suppression prévu).
  - *Constat 2026-09-17 (émulateur, destination dans le stockage interne via `ExternalStorageProvider`) :* changement de date d'une photo Téléphone + SSD → fichier rangé de `dcim/2026/09/11` vers `dcim/2026/09/14` (dossier existant réutilisé avec sa casse), copie identique à l'original, enregistrement `VERIFIED` et index mis à jour, `datetaken` recalculé par MediaStore. Le repli copie vérifiée + suppression est implémenté et testé unitairement. **À vérifier sur le Pixel avec le SSD USB** : déplacement réel (moveDocument ou repli), et coupure pendant un déplacement (reprise au lancement).
